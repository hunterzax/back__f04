import {Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Req, HttpException, HttpStatus, Put, UseInterceptors, UploadedFile, BadRequestException, Res, Query, Logger} from '@nestjs/common'
...
import {middleNotiInapp, middleNotiInappShipper, middleNotiInappTSOonly} from 'src/common/utils/inapp.util'
...
export class DailyManagementController {
  private readonly logger = new Logger(DailyManagementController.name)
  constructor(
    private readonly accountManageService: AccountManageService,
    private jwtService: JwtService,
    private readonly fileUploadService: FileUploadService,
    private readonly dailyManagementService: DailyManagementService,
  ...
  @UseGuards(AuthGuard)
  @Post('comment')
  async comments(@Req() req: any, @Body() body: any) {
    ...
    const comments = await this.queryShipperNominationFileService.comments(body, req?.user?.sub)

    try {
      const nom = await this.prisma.query_shipper_nomination_file?.findFirst({
        where: {
  ...
  @UseGuards(AuthGuard)
  @Post('edit-row/:id')
  async editRowJSON(@Req() req: any, @Body() body: any, @Param('id') id: any) {
    ...
    const editRowJSON = await this.queryShipperNominationFileService.editRowJSON(id, body, req?.user?.sub)
    ...
    try {
      ...
      const message = `Daily Nomination was editted for ${nom?.group?.name}:${nom?.contract_code?.contract_code} activate from ${dayjs(nom?.gas_day).format('DD/MM/YYYY')}`
      await middleNotiInappTSOonly(
        this.prisma,
        'Nomination',
        message,
      ...
      this.logger.error('editRowJSON err : ', error)
    }
  ...
  async updateStatus(@Req() req: any, @Body() body: any) {
    ...
    const updateStatus = await this.queryShipperNominationFileService.updateStatus(body, req?.user?.sub)

    const nom = await this.prisma.query_shipper_nomination_file?.findMany({
      where: {
        id: {
          in: id
        }
    ...
    try {
      for (let i = 0; i < id.length; i++) {
        ...
        const message = `Daily Nomination was ${status === 2 ? 'approved' : 'rejected'} for ${fnom?.group?.name}:${fnom?.contract_code?.contract_code} activate from ${dayjs(fnom?.gas_day).format('DD/MM/YYYY')} by operator`
        await middleNotiInappShipper(
          this.prisma,
          'Nomination',
          message,
    ...
    try {
      for (let i = 0; i < id.length; i++) {
        ...
        const header = status === 2 ? `TSO accepts the shipper` : `TSO rejects the shipper`
        const emailNotificationData = await emailNotificationDAM(this.prisma, status === 2 ? 9 : 11) // emailNotificationData?.subject subject: emailNotificationData?.subject || "", detail: emailNotificationData?.detail || "",
        const shipperEmailArr = await emailGetpermissionEmail(this.prisma, 61)
  ...
  exportFileNom(@Res() res: Response, @Body() Body: any, @Req() req: any) {
    return this.queryShipperNominationFileService.exportFileNom(res, Body, req?.user?.sub)
  }
}
