...
import {getTodayNowAdd7, getTodayNowDDMMYYYYAdd7, getTodayNowDDMMYYYYDfaultAdd7, getTodayStartDDMMYYYYAdd7} from 'src/common/utils/date.util'
...
export class ReleaseCapacitySubmissionService {
  ...
  async contractCode(userId: any) {
    ...
    const where_ = group?.user_type_id === 3 ? {
        group_id: group?.id,
  ...
  async getRelease(payload: any) {
    ...
    const convertDataFull = contractCode?.booking_version[0]?.booking_full_json[0]
    convertDataFull['data_temp'] = JSON.parse(contractCode?.booking_version[0]?.booking_full_json[0]['data_temp'])

    const convertData = (contractCode?.booking_version[0]?.booking_row_json || [])?.map((e: any) => {
      return {
        ...e,
        data_temp: JSON.parse(e['data_temp'])
      }
    ...
    const endDate = getTodayNowAdd7(contractCode?.terminate_date ?? contractCode?.extend_deadline ?? contractCode?.contract_end_date)
      ...
      return {
        ...
        contracted_mmbtu_d_array:
          keysMMBTU?.map((ks: any, kix: number) => {
            ...
            let edDayjs = kix === keysMMBTU.length - 1 ? dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY') : keysMMBTU.length > 0 ? dayjs(keysMMBTU[kix + 1]?.date, 'DD/MM/YYYY').subtract(1, 'day') : dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY').subtract(1, 'day')
        ...
        maximum_mmbtu_array:
          keysMMBTH?.map((ks: any, kix: number) => {
            ...
            let edDayjs = kix === keysMMBTH.length - 1 ? dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY') : keysMMBTH.length > 0 ? dayjs(keysMMBTH[kix + 1]?.date, 'DD/MM/YYYY').subtract(1, 'day') : dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY').subtract(1, 'day')
            if (edDayjs.isAfter(endDate)) {
              edDayjs = endDate.subtract(1, 'day')
            }
            const ed = edDayjs.format('DD/MM/YYYY')
        ...
        contracted_mmscfd_array:
          eSum['entry_exit_id'] === 1
            ? keysMMSCFD?.map((ks: any, kix: number) => {
                ...
                let edDayjs = kix === keysMMSCFD.length - 1 ? dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY') : keysMMSCFD.length > 0 ? dayjs(keysMMSCFD[kix + 1]?.date, 'DD/MM/YYYY').subtract(1, 'day') : dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY').subtract(1, 'day')
                if (edDayjs.isAfter(endDate)) {
                  edDayjs = endDate.subtract(1, 'day')
                }
                const ed = edDayjs.format('DD/MM/YYYY')
        ...
        maximum_mmscfd_array:
          eSum['entry_exit_id'] === 1
            ? keysMMSCFH?.map((ks: any, kix: number) => {
                ...
                let edDayjs = kix === keysMMSCFH.length - 1 ? dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY') : keysMMSCFH.length > 0 ? dayjs(keysMMSCFH[kix + 1]?.date, 'DD/MM/YYYY').subtract(1, 'day') : dayjs(eSum['data_temp'][fromTo + 1], 'DD/MM/YYYY').subtract(1, 'day')
                if (edDayjs.isAfter(endDate)) {
                  edDayjs = endDate.subtract(1, 'day')
                }
                const ed = edDayjs.format('DD/MM/YYYY')
  ...
  async getReleaseGroupByEntryAreaAndDate(payload: any) {
    ...
    const contractCodeEndDate = contractCode?.terminate_date ?? contractCode?.extend_deadline ?? contractCode?.contract_end_date
    ...
    convertData
      ...
      ?.map((entryPoint: any) => {
        ...
        if (isMatch(entryZone, 'East') || isMatch(entryZone, 'West')) {
          exitPoint = exitPoint.concat(
            convertData.filter((f: any) => {
              return isMatch(f?.zone_text, 'East-West') && f?.['entry_exit_id'] === 2
            })
          )
        }

        // เรียงลำดับจุดออกตามชื่อ contract_point
        exitPoint.sort((a, b) => a.contract_point.localeCompare(b.contract_point))
        ...
        if (!groupByEntryAreaWithSameZone[key]) {
          ...
          groupByEntryAreaWithSameZone[key] = [...groupByEntryAreaWithSameZone[key], entryPoint, ...exitPoint].sort((a, b) => {
    ...
    while (unUsedExitPointList.length > 0) {
      unUsedExitPointList?.map((exitPoint: any) => {
        ...
        let pointList = unUsedExitPointList.filter((f: any) => isMatch(f?.zone_text, exitZone))

        // หากเป็นโซน East หรือ West ให้รวมจุดออกในโซน East-West ด้วย
        if (isMatch(exitZone, 'East') || isMatch(exitZone, 'West')) {
          pointList = pointList.concat(unUsedExitPointList.filter((f: any) => isMatch(f?.zone_text, 'East-West')))
        }

        // เรียงลำดับตามชื่อ contract_point
        pointList.sort((a, b) => a.contract_point.localeCompare(b.contract_point))
        ...
        if (!groupByEntryAreaWithSameZone[key]) {
          ...
          groupByEntryAreaWithSameZone[key] = [...groupByEntryAreaWithSameZone[key], ...pointList].sort((a, b) => a.contract_point.localeCompare(b.contract_point))
        }
    ...
    Object.keys(groupByDate)?.map((groupkey) => {
      ...
      const tempDates = groupData?.map((data: any) => data?.map((item: any) => item.temp_date)).flat()
      ...
      const minDate = tempDates.reduce((min: dayjs.Dayjs, current: dayjs.Dayjs | null) => (getTodayNowDDMMYYYYAdd7(current) < min ? getTodayNowDDMMYYYYAdd7(current) : min), defaultDate)
      const maxDate = tempDates.reduce((max: dayjs.Dayjs, current: dayjs.Dayjs | null) => (getTodayNowDDMMYYYYAdd7(current) > max ? getTodayNowDDMMYYYYAdd7(current) : max), defaultDate)
      ...
      if (minDate && maxDate) {
        ...
        while (currentMonth.isBefore(endMonth) || currentMonth.isSame(endMonth, 'month')) {
          ...
          const monthStart = minDate.isSame(currentMonth, 'month') && minDate.isAfter(startOfMonth) ? minDate : startOfMonth
          const monthEnd = maxDate.isSame(currentMonth, 'month') && maxDate.isBefore(endOfMonth) ? maxDate : endOfMonth
          ...
          data.push(
            groupData[0]?.map((item: any) => {
              return {
                ...item,
                temp_start_date: monthStart?.tz('Asia/Bangkok')?.format('DD/MM/YYYY') ?? item.temp_date, // วันที่เริ่มต้นของเดือน
                temp_end_date: monthEnd?.tz('Asia/Bangkok')?.format('DD/MM/YYYY') ?? item.temp_date, // วันที่สิ้นสุดของเดือน
                temp_from_date: monthEnd?.tz('Asia/Bangkok')?.format('DD/MM/YYYY') ?? item.temp_date, // วันที่จาก (ใช้ค่าเดียวกับ temp_end_date)
                temp_to_date: monthEnd?.tz('Asia/Bangkok')?.format('DD/MM/YYYY') ?? item.temp_date, // วันที่ถึง (ใช้ค่าเดียวกับ temp_end_date)
                temp_date: startOfMonth.tz('Asia/Bangkok')?.format('DD/MM/YYYY') ?? item.temp_date // วันที่ของเดือน (วันที่ 1)
              }
            })
          )
    ...
    const sortedData = data.sort((a, b) => {
      try {
        ...
        return firstItemA.temp_contract_point.localeCompare(firstItemB.temp_contract_point)
      } catch (error) {
  ...
  async createSubmission(contract_code_id: any, group_id: any, data: any, userId: any, status: any) {
    ...
    const newData = await data?.map((e: any) => {
      ...
      return {
        ...
        total_contracted_mmbtu_d: (!!total_contracted_mmbtu_d && String(total_contracted_mmbtu_d)) || null,
        total_release_mmbtu_d: (!!total_release_mmbtu_d && String(total_release_mmbtu_d)) || null,
        ...
        total_contracted_mmscfd: (!!total_contracted_mmscfd && String(total_contracted_mmscfd)) || null,
        total_release_mmscfd: (!!total_release_mmscfd && String(total_release_mmscfd)) || null,
        temp_start_date: temp_start_date ? getTodayNowDDMMYYYYDfaultAdd7(temp_start_date).toDate() : null,
        temp_end_date: temp_end_date ? getTodayNowDDMMYYYYDfaultAdd7(temp_end_date).toDate() : null,
    ...
    if (status === 2) {
      const result = await this.prisma.$transaction(
        async (prisma) => {
          ...
          const {contract_code_id, group_id, release_capacity_submission_detail, submission_time} = getRelease
          ...
          for (let i = 0; i < release_capacity_submission_detail.length; i++) {
            dataArr.push({
              ...
              booking_row_json_id: Number(release_capacity_submission_detail[i]?.booking_row_json_id),
              path_management_config_temp: release_capacity_submission_detail[i]?.path_management_config_temp,
              path_management_config_id: Number(release_capacity_submission_detail[i]?.path_management_config_id),
              entry_exit_id: Number(release_capacity_submission_detail[i]?.entry_exit_id),
              ...
              //   },
              // }),
              temp_contract_point: release_capacity_submission_detail[i]?.temp_contract_point,
              temp_area: release_capacity_submission_detail[i]?.temp_area,
              temp_zone: release_capacity_submission_detail[i]?.temp_zone,
              total_contracted_mmbtu_d: (release_capacity_submission_detail[i]?.total_contracted_mmbtu_d && String(release_capacity_submission_detail[i]?.total_contracted_mmbtu_d)) || null,
              total_release_mmbtu_d: (release_capacity_submission_detail[i]?.total_release_mmbtu_d && String(release_capacity_submission_detail[i]?.total_release_mmbtu_d)) || null,
              total_contracted_mmscfd: (release_capacity_submission_detail[i]?.total_contracted_mmscfd && String(release_capacity_submission_detail[i]?.total_contracted_mmscfd)) || null,
              total_release_mmscfd: (release_capacity_submission_detail[i]?.total_release_mmscfd && String(release_capacity_submission_detail[i]?.total_release_mmscfd)) || null,
              release_start_date: release_capacity_submission_detail[i]?.temp_start_date || null,
              release_end_date: release_capacity_submission_detail[i]?.temp_end_date || null,
              create_date: dateCre.toDate(),
              create_by: Number(userId),
              create_date_num: getTodayNowAdd7().unix()
              // create_by_account: {
          ...
          releaseDa['release_capacity_submission_detail'] = releaseDa['release_capacity_submission_detail']?.map((e: any) => {
            e['path_management_config_temp'] = JSON.parse(e['path_management_config_temp'])
            e['path_management_config']['temps'] = JSON.parse(e['path_management_config']['temps'])
            return e
          })
          // ..........
          ...
          const strl = await this.stampRelease(releaseDa?.release_capacity_submission_detail?.map((ns: any) => ns['path_management_config_temp']) || [], releaseDa?.contract_code_id, prisma)
            ...
            throw new HttpException(
              {
                ...
                error: 'Release Value Alert: The Release value must not exceed the Contract value.'
              },
  ...
  async stampRelease(data: any, contract_code_id: any, prisma?: Omit<PrismaClient<Prisma.PrismaClientOptions, Prisma.LogLevel, DefaultArgs>, '$connect' | '$disconnect' | '$on' | '$transaction' | '$use' | '$extends'>) {
    ...
    const npathManagementArr = pathManagementArr?.map((p: any) => {
      ...
      const npathConfig = npath_management_config.map((e: any) => {
        ...
        const findExit = e?.temps?.revised_capacity_path?.map((tp: any) => {
          return {
            ...tp,
            source_id: e?.temps?.revised_capacity_path_edges?.find((f: any) => f?.target_id === tp?.area?.id)?.source_id || null
          }
        })
        return {
          ...e,
    ...
    const fPub = capaDetail?.capacity_detail_point?.find((f: any) => f?.capacity_detail_point_date.length > 0)
    ...
    for (let i = 0; i < data.length; i++) {
      const dates = genDateRangeDDMMYYYY(data[i]?.temp_start_date, data[i]?.temp_end_date)

      let nDates = dates?.flatMap((e: any) => {
        let dataA = []
        if (data[i]?.entry_exit_id === 1) {
          ...
          const exitMainA = pathIdMaster?.filter((f: any) => {
            return f?.exit_name_temp === data[i]?.temp_area && path_temp_json?.map((pj: any) => pj?.pathConfig?.path_id)?.includes(f?.id)
          })

          for (let ix = 0; ix < exitMainA.length; ix++) {
            const areaPathA = path_temp_json?.filter((f: any) => f?.pathConfig?.path_id === exitMainA[ix]?.id) || []
            for (let ip = 0; ip < areaPathA.length; ip++) {
              let arrArea = []
              arrArea.push(areaPathA[ip]?.pathConfig?.findExit?.find((f: any) => f?.source_id === null))
              for (let ix = 0; ix < areaPathA[ip]?.pathConfig?.findExit?.length; ix++) {
              ...
              const areaArrExit = (arrArea || [])?.filter((f: any) => f?.source_id !== null)
              ...
              let dataA0 = nAreaExit?.map((nAe: any) => {
                const nCapacityPointDateFind = nCapacityPointDate?.find((f: any) => {
                  return f?.dateFormate === e && f?.area_id === nAe?.area_id && f?.path_id === nAe?.path_id
                })
              ...
              dataA = [...dataA, ...dataA0?.filter((f: any) => f?.capacityPointDate !== null)]
            }
          }
        }
    ...
    const usePathDetail = nDateArr?.map((pDetail: any) => {
      return {
        ...
        release_old: (pDetail?.capacityPointDate?.release && Number(pDetail?.capacityPointDate?.release)) || null,
    ...
    const nresultGroupId = resultGroupId?.map((iGp: any) => {
      ...
      const calcSumRelease = iGp?.release_old !== null ? Number(iGp?.release_old) + Number(sumNew) : Number(sumNew)
      return {
        id: iGp?.id,
    ...
    for (let iPd = 0; iPd < nresultGroupId.length; iPd++) {
      if (nresultGroupId[iPd]?.release) {
        await this.prisma.capacity_detail_point_date.updateMany({
          where: {
            id: Number(nresultGroupId[iPd]?.id ?? -1)
          },
          data: {
            release: (nresultGroupId[iPd]?.release && String(nresultGroupId[iPd]?.release)) || null
          }
        })
      }
    }
    ...
    const nresultGroupPubArea = resultGroupPubArea?.map((nG: any) => {
      ...
      for (const curr of dataN) {
        ...
        groupedPubAreaDate[key].data.push({
          ...curr
        })
      }
      const resultGroupPubAreaDate: any = Object.values(groupedPubAreaDate)
      const nresultGroupPubAreaDate = (resultGroupPubAreaDate || [])?.map((ad: any) => {
    ...
    const capaPublich = await (prisma ?? this.prisma).capacity_publication_date.findMany({
      where: {
    ...
    const ncapaPublich = capaPublich
      ?.map((nc: any) => {
        const fcapacity_publication = fUsePub?.find((f: any) => {
          return f?.id_area === nc?.capacity_publication?.area_id && f?.date === dayjs(nc?.date_day).format('DD/MM/YYYY')
        })
        return {
          id: nc?.id,
          old_value: nc?.value,
          ...
          value: parseToNumber(nc?.value) ? String(parseToNumber(nc?.value) + (fcapacity_publication?.val || 0)) : String(0 + (fcapacity_publication?.val || 0)),
          value_adjust_use: parseToNumber(nc?.value_adjust_use) ? String((parseToNumber(nc?.value_adjust_use) || 0) + parseToNumber(fcapacity_publication?.val || 0)) : String(((nc?.value && parseToNumber(nc?.value)) || 0) + (fcapacity_publication?.val || 0)),
    ...
    for (let iPd = 0; iPd < ncapaPublich.length; iPd++) {
      await this.prisma.capacity_publication_date.updateMany({
        where: {
          id: Number(ncapaPublich[iPd]?.id ?? -1)
        },
        data: {
          value: (ncapaPublich[iPd]?.value && String(ncapaPublich[iPd]?.value)) || null,
          value_adjust_use: (ncapaPublich[iPd]?.value_adjust_use && parseToNumber(ncapaPublich[iPd]?.value_adjust_use) && String(ncapaPublich[iPd]?.value_adjust_use)) || null
        }
      })
    }
  ...
  async submission(payload: any, userId: any) {
    ...
    if (group?.user_type_id === 3) {
      // shipper
      const submiss = await this.createSubmission(contract_code_id, group_id, data, userId, 1)
      url !== null &&
        url.length > 0 &&
      ...
      // tso approved
      const submiss = await this.createSubmission(contract_code_id, group_id, data, userId, 2)
      url !== null &&
        url.length > 0 &&
        (await this.submissionFileCreate(
          {
  ...
  async documentFile(id: any) {
    const resData = await this.prisma.release_capacity_submission_file_document.findMany({
      where: {
        active: true,
  ...
  async documentFileCreate(payload: any, userId: any) {
    ...
    const resData = await this.prisma.release_capacity_submission_file_document.create({
      data: {
        contract_code: {
          connect: {
            id: Number(contract_code_id)
  ...
  async documentFileInactive(payload: any, userId: any) {
    const {id} = payload
    const resData = await this.prisma.release_capacity_submission_file_document.update({
      where: {
        id: Number(id)
      },
      data: {
  ...
  async getApprovedReleaseCapacitySubmissionDetail(contract_code_id: any) {
    try {
      ...
      const resData = await this.prisma.release_capacity_submission_detail.findMany({
        where: {
          release_capacity_submission: {
